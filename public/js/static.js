/**
 * @depends jquery-1.7.1.min.js
 * @depends jquery-ui.min.js
 * @depends jquery.ui.widget.js
 * @depends jquery.validate.min.js
 * @depends superfish.js
 * @depends jquery.ez-pinned-footer.js
 * @depends jquery.ksfunctions.js
 * @depends tmpl.min.js
 * @depends load-image.min.js
 * @depends canvas-to-blob.min.js
 * @depends bootstrap.min.js
 * @depends bootstrap-image-gallery.min.js
 * @depends jquery.iframe-transport.js
 * @depends jquery.fileupload.js
 * @depends jquery.fileupload-fp.js
 * @depends jquery.fileupload-ui.js
 * @depends locale.js
 * @depends admin.js
 */